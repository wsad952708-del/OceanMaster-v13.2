"""
OceanMaster v13.2 — 月相效應引擎 (Lunar Phase Engine)
====================================================
...
LUNAR_CPUE_MODIFIERS = {
    ...
    ("squid", "squid_jigging"):   (1.50, 0.40),
    ("neon_flying_squid", "squid_jigging"): (1.50, 0.40),
    ("japanese_flying_squid", "squid_jigging"): (1.50, 0.40),
...
class LunarPhaseEngine:
    ...
    def compute_surface_illuminance(
        self,
        illumination: float,
        cloud_cover: float = 0.0,
        solar_altitude: float = -90.0,
    ) -> Dict:
        """
        [v16.0] True surface illuminance — 校正月光被雲層削弱的真實光照。

        月相照明度 × (1 - cloud_cover) × atmospheric_transmittance
        用途：魷釣最佳開燈時機判斷。

        Args:
            illumination: 0-1 月面照亮率 (from compute_moon_phase)
            cloud_cover: 0-1 雲覆蓋率 (from Open-Meteo, already in weather data)
            solar_altitude: 太陽高度角 (deg, <0 = 夜晚)

        Returns:
            {
                "surface_lux": float (estimated surface illuminance in lux),
                "true_darkness": float (0-1, 1=完全黑暗),
                "squid_lamp_advice": str,
            }
        """
        # Full moon at zenith ≈ 0.25 lux, new moon ≈ 0.001 lux
        # Ref: Kyba et al. (2017) Scientific Reports
        base_lunar_lux = 0.001 + 0.249 * illumination

        # Cloud attenuation: thick cloud blocks ~90% of moonlight
        cloud_transmit = max(0.1, 1.0 - 0.9 * cloud_cover)

        # Atmospheric extinction (altitude/airmass simplified)
        atm_transmit = 0.85  # typical clear maritime atmosphere

        # Only relevant at night (solar_altitude < -6° = civil twilight)
        if solar_altitude > -6:
            # Twilight or daytime — no need for lunar illuminance
            surface_lux = 100.0  # arbitrary bright
            darkness = 0.0
        else:
            surface_lux = base_lunar_lux * cloud_transmit * atm_transmit
            # True darkness: 0=bright, 1=pitch black
            darkness = max(0.0, 1.0 - surface_lux / 0.25)

        # Squid lamp advice
        if darkness > 0.8:
            advice = "🌑 極暗夜晚，全力開燈效果最佳"
        elif darkness > 0.5:
            advice = "🌓 中等光照，建議半功率開燈"
        elif darkness > 0.2:
            advice = "🌕 月光較亮，集魚燈效果降低"
        ...
        return {
            "surface_lux": round(float(surface_lux), 4),
            "true_darkness": round(float(darkness), 3),
            "cloud_cover": round(float(cloud_cover), 2),
            "squid_lamp_advice": advice,
        }
