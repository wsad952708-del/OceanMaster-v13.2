"""
OceanMaster v13.2 — 商業核心引擎 (Commercial Core Engine)
============================================================
v10.2: 已確認全部被 main_v10_2.py 呼叫（解決 v10.1 死代碼問題）
...
# Sources per species:
#   skipjack:  Eo from Graham & Dickson 2004; Pcrit from Brill 1994
#   yellowfin: Eo from Deutsch 2015 Fig.2 extrapolation; Pcrit from Brill 1994
#   bigeye:    Eo from Deutsch 2015 Fig.2 extrapolation; Pcrit from Brill 1994
#              (low Pcrit reflects rete mirabile adaptation)
#   albacore:  estimated from Blank et al. 2004 oxygen consumption data
#   squid spp: Eo from Seibel 2007 (cephalopod metabolic scaling)
#   pacific_saury: estimated from Watanabe & Oozeki 2005 + Deutsch 2015 range
METABOLIC_TRAITS = {
    "skipjack": {
        "Eo": 0.45,          # 高溫敏感（熱帶表層物種）— Graham & Dickson 2004
        "Pcrit_kPa": 5.5,    # 相對高的氧需求 — Brill 1994
        "Topt_C": 26.0,      # 最適溫度 — Barkley et al. 1978
        "Topt_sigma": 3.5,   # 溫度容忍寬度
        "phi_crit": 3.0,     # 臨界 Φ（需要高於靜息的 3 倍才能活動覓食）
        "depth_pref_m": [0, 50, 100, 150],
    },
    "yellowfin": {
        "Eo": 0.40,          # Deutsch 2015 Fig.2 extrapolation
        "Pcrit_kPa": 4.5,    # Brill 1994
        "Topt_C": 24.0,      # Sund et al. 1981
        "Topt_sigma": 4.0,
        "phi_crit": 2.8,
        "depth_pref_m": [0, 50, 100, 200],
    },
    "bigeye": {
        "Eo": 0.30,          # 低溫敏感（深潛物種，rete mirabile）— Deutsch 2015 extrapolation
        "Pcrit_kPa": 3.8,    # [v13.2-GH2] 統一 species_params.py=3.8 (Brill 1994 + Lowe 2000)
        "Topt_C": 18.0,      # 偏好較冷水 — Hanamoto 1987
        "Topt_sigma": 6.0,   # 寬溫度範圍 — Holland et al. 1999
        "phi_crit": 2.0,
        "depth_pref_m": [50, 100, 200, 300, 400],
        "source_verified": True,  # [v13.2-GH2] cadeutsch/Metabolic-Index-Traits 比對
    },
    "albacore": {
        "Eo": 0.38,          # estimated from Blank et al. 2004
        "Pcrit_kPa": 4.0,    # estimated from Blank et al. 2004
        "Topt_C": 19.0,      # Arrizabalaga et al. 2015
        "Topt_sigma": 3.0,
        "phi_crit": 2.5,
        "depth_pref_m": [0, 50, 100, 200, 300],
    },
    "squid_todarodes": {
        "Eo": 0.50,          # 魷魚代謝率極高 — Seibel 2007
        "Pcrit_kPa": 5.0,    # Seibel 2007
        "Topt_C": 16.0,
        "Topt_sigma": 4.0,
        "phi_crit": 3.5,
        "depth_pref_m": [0, 50, 100, 200],
    ...
    "squid_ommastrephes": {
        "Eo": 0.48,          # Seibel 2007 (cephalopod metabolic scaling)
        "Pcrit_kPa": 4.8,
        "Topt_C": 22.0,
        "Topt_sigma": 5.0,
        "phi_crit": 3.2,
    ...
    "pacific_saury": {
        "Eo": 0.35,          # estimated from Watanabe & Oozeki 2005 + Deutsch 2015
        "Pcrit_kPa": 4.0,
        "Topt_C": 15.0,        # 秋刀魚偏好冷水 — Suyama et al. 2012
        "Topt_sigma": 3.5,
        "phi_crit": 2.5,
        "depth_pref_m": [0, 30, 50],
    },
    ...
    # [v13.2-GH2] 新增 — 太平洋巨魷 (Humboldt Squid)
    "dosidicus_gigas": {
        "Eo": 0.55,          # Rosa & Seibel 2008 (極高代謝率+低溫耐受)
        "Pcrit_kPa": 3.0,    # Rosa & Seibel 2008 (能在 OMZ 覓食)
        "Topt_C": 20.0,      # Gilly et al. 2006
        "Topt_sigma": 6.0,   # 極寬溫度範圍 (5-30°C)
        "phi_crit": 2.0,     # 低要求 — 能承受嚴苛缺氧環境
        "depth_pref_m": [0, 100, 200, 400, 600],
...
class MetabolicIndexEngine:
    ...
    def phi_to_habitat_viability(
        ...
        k = 5.0 / max(phi_crit, 1.0)  # 斜率 (v10.2.1: 3.0→5.0 更陡峭)

        viability = 1.0 / (1.0 + np.exp(-k * (phi - phi_crit)))
        return np.clip(viability, 0.0, 1.0).astype(np.float32)

    @staticmethod
    def compute_p50_depth(
        temp_profile: np.ndarray,
        do_profile: np.ndarray,
        depths: list,
        species: str,
        salinity_profile: Optional[np.ndarray] = None,
    ) -> Dict[str, Any]:
        """
        [v13.2-GH7] P50 垂直棲地深度計算

        P50 = 血液半飽和氧分壓 (kPa)
        Source: kallisons/CMIP5_p50_tuna (github.com)
        Paper: Global Change Biology

        P50 比 Pcrit 更精確:
          - Pcrit = 靜息代謝的最低氧需求（存活閾值）
          - P50 = 血紅蛋白 50% 飽和的 pO₂（活動能力閾值）

        方法：沿深度剖面計算 pO₂，找到 pO₂ = P50 的深度
        → 該深度以下，鮪魚無法維持血液充足供氧
        → 定義了生理上的最大活動深度

        P50 值 (kPa, from Lowe et al. 2000, Brill & Bushnell 2001):
          skipjack:  2.5  (高 P50 → 淺層限制)
          yellowfin: 2.8
          bigeye:    1.5  (低 P50 → 能深潛，rete mirabile)
          albacore:  2.2

        Returns:
            p50_depth: P50 等值深度 (m)，-1 if 全水柱都夠
            vertical_habitat_range: (最淺, P50深度) 可活動範圍
            oxygen_squeeze: 0-1 缺氧壓縮指數
        """
        # P50 值 (kPa) — 血紅蛋白半飽和氧分壓
        P50_VALUES = {
            "skipjack": 2.5,
            "yellowfin": 2.8,
            "bigeye": 1.5,
            "albacore": 2.2,
            "neon_flying_squid": 3.0,      # 魷魚血藍蛋白 P50 較高
            "japanese_flying_squid": 3.0,
            "dosidicus_gigas": 1.8,         # OMZ forager, very low P50
            "pacific_saury": 2.5,
            "mahi_mahi": 2.8,
            "blue_marlin": 2.0,
        }

        p50 = P50_VALUES.get(species, 2.5)
        mi = MetabolicIndexEngine()
        ...
        for i, d in enumerate(depths):
            if i >= len(temp_profile) or i >= len(do_profile):
                break
            t = temp_profile[i] if np.isscalar(temp_profile[i]) else float(np.nanmean(temp_profile[i]))
            do = do_profile[i] if np.isscalar(do_profile[i]) else float(np.nanmean(do_profile[i]))
            s = salinity_profile[i] if salinity_profile is not None else 35.0
            if np.isnan(t) or np.isnan(do):
                po2_profile.append(np.nan)
                continue

            po2 = mi.do_to_po2(
                np.array([do]),
                np.array([t]),
                np.array([s]) if not np.isscalar(s) else np.array([float(s)])
            )[0]
            po2_profile.append(float(po2))

            # 找 pO₂ 第一次降到 P50 以下的深度
            if po2 < p50 and p50_depth < 0:
                # 線性插值找精確深度
                if i > 0 and len(po2_profile) > 1 and not np.isnan(po2_profile[-2]):
                    po2_above = po2_profile[-2]
                    d_above = depths[i - 1]
                    if po2_above > p50:
                        frac = (po2_above - p50) / max(po2_above - po2, 0.01)
                        p50_depth = d_above + frac * (d - d_above)
                    else:
                        p50_depth = float(d)
                else:
                    p50_depth = float(d)

        # 氧壓縮指數 (Oxygen Squeeze)
        # = 可用深度範圍 / 物種偏好深度範圍
        traits = METABOLIC_TRAITS.get(species, METABOLIC_TRAITS.get("yellowfin", {}))
        pref_depths = traits.get("depth_pref_m", [0, 300])
        max_pref = max(pref_depths)

        if p50_depth < 0:
            # 全水柱都夠 → 無壓縮
            oxygen_squeeze = 0.0
            vertical_range = (0, max(depths))
        else:
            oxygen_squeeze = max(0, 1.0 - p50_depth / max(max_pref, 1))
            vertical_range = (0, p50_depth)

        return {
            "p50_depth_m": round(p50_depth, 1),
            "p50_kpa": p50,
            "po2_profile": po2_profile,
            "vertical_habitat_range_m": vertical_range,
            "oxygen_squeeze": round(max(0, min(1, oxygen_squeeze)), 3),
            "species": species,
        }
...
class SEAPODYMHabitatEngine:
    ...
    def compute_combined_habitat(
        ...
        """
        # 核心三因子（幾何平均）
        # Weights from Lehodey 2008 SEAPODYM structure, manually calibrated
        # against WCPFC 2010-2018 longline CPUE spatial correlation
        w_thermal = 0.25
        w_feeding = 0.30
        w_phi = 0.25
    ...
    def compute_ultimate_hsi(
        ...
        try:
            ...
            if do_surface is not None and np.any(do_surface > 0):
                data_quality_score += 0.15
            data_quality_grade = "?"
            dq = {"score": data_quality_score, "grade": data_quality_grade,
                  "note": "fallback — calibration module 不可用"}
        ...
        log.info(f"    HSI={mean_hsi:.3f} | Φ={mean_phi:.2f} | "
                 ...
                 f"data_quality={data_quality_score:.0%} ({data_quality_grade})")
        ...
        return {
            ...
            "front_persistence": front_persist,
            "eddy_edge": eddy_edge_idx,
            "confidence": data_quality_score,  # 向後相容
            "data_quality": dq,  # 新欄位: 包含 score/grade/breakdown/note
        }
