"""
OceanMaster v13.2 — 異常值偵測 + 氣候指數引擎
================================================
...
class SSTClimatologyCache:
    ...
    def _download_oisst_clim(self, month: int, cache_file: str):
        """[STUB] OISST 氣候態下載 — 尚未實做。

        目前總是回退至 _fallback_formula()。
        未來可接入 NOAA ERDDAP ncdcOisst21Agg_LonPM180
        (1991-2020 基準期) 以取得真實月均氣候態。
        """
        # [v13.2-audit] 移除假裝下載的死代碼，誠實標記為未實做
        log.debug(f"OISST 氣候態下載尚未實做 (m={month})，使用公式回退")
        return None
...
class TemporalLagEngine:
    ...
    def compute_lagged_bloom_potential(
        ...
        # [v13.2-audit] guard against int(nan) when drift_grids is all NaN
        _drift_mean = np.nanmean(drift_grids)
        kernel = max(3, int(_drift_mean if np.isfinite(_drift_mean) else 1) * 2 + 1)
