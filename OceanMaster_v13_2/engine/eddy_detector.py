"""
OceanMaster v13.2 — SSH 渦旋偵測 + 渦旋邊緣追蹤  # [v12-enhance]
==========================================
...
class EddyDetector:
    ...
    def __init__(self, ow_threshold: float = -2.0):
        ...
        # [v19-causal] 渦旋追蹤狀態
        # 儲存上一幀的渦旋重心和年齡，用於跨幀追蹤
        self._prev_eddy_centroids = {}  # {eddy_id: (lat, lon, age_days)}
    ...
    def detect(self, ssh: np.ndarray,
        ...
        # ── [逆向工程成果] 渦旋特徵擴展 ──────────────────
        # 海鷹 AI 2024 最新論文用 SHAP 發現渦旋半徑是
        # 長鰭金枪魚第三重要因子 (SST > DO > eddy_radius)

        # 1. 渦旋等效半徑 (km) — 從每個連通區域計算
        eddy_radius_km = np.zeros((ny, nx), dtype=np.float32)
        try:
            from scipy.ndimage import label
            labeled, n_eddies = label(eddy_core)
            grid_area_km2 = (mean_dx / 1000.0) * (mean_dy / 1000.0)
            for eid in range(1, n_eddies + 1):
                emask = labeled == eid
                area_km2 = np.sum(emask) * grid_area_km2
                radius = np.sqrt(area_km2 / np.pi)  # 等效圓半徑
                eddy_radius_km[emask] = radius
            log.info(f"  渦旋半徑: {n_eddies} 個渦旋, "
                     f"max={np.max(eddy_radius_km):.1f} km")
        except ImportError:
            log.warning("  scipy 不可用，跳過渦旋半徑計算")

        # 2. 核心區域動能 (只在渦旋核心內有值)
        eke_core = eke * eddy_core.astype(np.float32)

        # ── [v19-causal] 渦旋年齡追蹤 ──────────────────
        # 因果鏈 #2: 赤魷豐度在渦旋成熟期(生命中段)達到 peak
        # 使用跨幀重心匹配追蹤渦旋壽命
        # 參考: Yu et al. (2023) Frontiers in Marine Science
        eddy_age_days = np.zeros((ny, nx), dtype=np.float32)
        try:
            if 'labeled' not in dir():
                from scipy.ndimage import label as _label
                labeled, n_eddies = _label(eddy_core)
            
            new_centroids = {}
            for eid in range(1, n_eddies + 1):
                emask = labeled == eid
                # 計算渦旋重心 (lat, lon)
                y_idx, x_idx = np.where(emask)
                if len(y_idx) == 0:
                    continue
                centroid_lat = float(np.mean(lats[y_idx])) if y_idx.max() < len(lats) else float(np.mean(y_idx))
                centroid_lon = float(np.mean(lons[x_idx])) if x_idx.max() < len(lons) else float(np.mean(x_idx))
                
                # 與上一幀的渦旋匹配 (距離 < 200 km = 同一渦旋)
                best_match_id = None
                best_dist = 200.0  # km 閾值
                for prev_id, (prev_lat, prev_lon, prev_age) in self._prev_eddy_centroids.items():
                    dlat = (centroid_lat - prev_lat) * 111.0
                    dlon = (centroid_lon - prev_lon) * 111.0 * np.cos(np.radians(centroid_lat))
                ...
                if best_match_id is not None:
                    # 匹配成功：延續前一幀年齡 +1 天
                    age = self._prev_eddy_centroids[best_match_id][2] + 1.0
                else:
                    # 無匹配：新生渦旋，年齡 = 1
                    age = 1.0
                
                new_centroids[eid] = (centroid_lat, centroid_lon, age)
                eddy_age_days[emask] = age
            
            # 更新追蹤狀態
            self._prev_eddy_centroids = new_centroids
            
            if n_eddies > 0:
                max_age = np.max(eddy_age_days)
                log.info(f"  🔄 渦旋追蹤: {n_eddies} 個渦旋, "
                         f"最大年齡={max_age:.0f}天, "
                         f"成熟度指數={min(max_age/90, 1.0):.2f}")
        except Exception as e:
            log.debug(f"  渦旋追蹤失敗 (非關鍵): {e}")
        ...
        return {
            ...
            # [逆向工程新增]
            "eddy_radius_km": eddy_radius_km,
            "eddy_kinetic_energy_core": eke_core.astype(np.float32),
            # [v19-causal] 渦旋年齡 (天) — 用於計算 eddy_maturity_index
            "eddy_age_days": eddy_age_days,
        }
    ...
    def compute_eddy_biological_enrichment(
        ...
        # 背景 Chl-a (渦旋外均值)
        eddy_bool = eddy_core.astype(bool)
        non_eddy_mask = ~eddy_bool & np.isfinite(chl)
