...
class ForageEngine:
    ...
    def compute_npp_vgpm(self, chl: np.ndarray, sst: np.ndarray,
        ...
        npp = np.clip(npp, 0, 2500)  # [v16.0] MODIS open-ocean range: 0-2500
    ...
    def compute(self, chl: np.ndarray, sst: np.ndarray,
        ...
        # [v13.2-P1] NPZ 生態動力模型補充
        try:
            from engine.npz_model import NPZModel
            npz = NPZModel()
            npz_result = npz.run(sst, chl, par=par, n_days=15)
            result["npz_phytoplankton"] = npz_result["phytoplankton"]
            result["npz_zooplankton"] = npz_result["zooplankton"]
            result["npz_nutrients"] = npz_result["nutrients"]
            log.info(f"  NPZ: P={np.nanmean(npz_result['phytoplankton']):.2f}, "
                     f"Z={np.nanmean(npz_result['zooplankton']):.2f}")
        except Exception as e:
            log.debug(f"  NPZ model not available: {e}")
