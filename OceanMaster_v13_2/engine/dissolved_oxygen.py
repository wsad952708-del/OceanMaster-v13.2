    ...
    def _fetch_woa_netcdf4(
        ...
        try:
            ...
            try:
                ...
                log.debug(f"[降級] engine/dissolved_oxygen.py: {e}")
    ...
    def _generate_woa_climatology(
        ...
        for i, depth in enumerate(WOA_STANDARD_DEPTHS):
            if depth <= 50:
                ...
                # 近岸上升流區 DO 稍高（營養鹽驅動）
                # [v13.2-fix] 確定性空間擾動取代 np.random（可再現性）
                perturbation = 0.03 * np.sin(lat_grid * 7.3 + lon_grid * 11.1 + depth * 0.01)
                do_profile[i] = np.clip(base_do + perturbation, 2.0, 8.5)
