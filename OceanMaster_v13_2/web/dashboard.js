// ═══════════════════════════════════════════════════════════
// OceanMaster v15.3 — Commercial Maritime Dashboard
...
// ── Map ──
// [v13.2-R9] Guard: if Leaflet CDN failed, skip map entirely
let map, tileMain, zoneLayer, hotspotLayer, labelLayer, typhoonLayer;
if (typeof L !== 'undefined' && !window.__leafletFailed) {
    map = L.map('map', { zoomControl: false, attributionControl: false }).setView([25, 132], 5);
    L.control.zoom({ position: 'bottomright' }).addTo(map);
    // [v13.2-R7] Primary tile layer with offline fallback
    tileMain = L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
        maxZoom: 19, subdomains: 'abcd',
        errorTileUrl: ''
    }).addTo(map);
    // Fallback: if CDN tiles fail, switch to OSM
    let _tileErrors = 0;
    tileMain.on('tileerror', () => {
        _tileErrors++;
        if (_tileErrors > 4 && !map._fallbackTiles) {
            map._fallbackTiles = true;
            map.removeLayer(tileMain);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                maxZoom: 19, attribution: '© OSM'
            }).addTo(map);
            console.warn('CartoDB tiles unavailable, switched to OSM fallback');
        }
    });
    ...
    console.warn('[v13.2-R9] Leaflet unavailable — map disabled, data-only mode');
    document.getElementById('map').innerHTML =
        '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#94a3c0;font-size:14px">' +
        '🗺️ 地圖載入失敗（無法連線 CDN），漁場資料仍可使用</div>';
}
let sstLayer = null, eezLayer = null;
if (typeof L !== 'undefined') {
    sstLayer = L.layerGroup();
    eezLayer = L.layerGroup();
    if (map) eezLayer.addTo(map);
}
...
function _hideLoading() {
    // [v13.2-R9] Use the global failsafe if available
    if (window.__hideLoading) { window.__hideLoading(); return; }
...
async function loadAll() {
    // [v13.2-R9] 5-second hard timeout (matches HTML failsafe)
    const forceTimer = setTimeout(() => {
        console.warn('loadAll: 5s timeout reached, forcing loading screen hide');
        _hideLoading();
    }, 5000);
    try {
        await Promise.all([loadHotspots(), loadSea(), loadTyphoons()]);
    } catch (e) {
...
async function loadHotspots() {
    try {
        // [v13.2-R7] 8s fetch timeout to prevent hanging
        const ctrl = new AbortController();
        const tid = setTimeout(() => ctrl.abort(), 8000);
        const r = await fetch('/api/hotspots', { headers: authH(), signal: ctrl.signal });
        clearTimeout(tid);
        if (!r.ok) throw new Error(r.status);
        _hideLoading();  // [v13.5] Hide loading immediately on successful API response
        const d = await r.json();
        allHotspots = d.hotspots || [];
        // [v13.9] Debug: print first hotspot's full JSON for field verification
        if (allHotspots.length > 0) {
            console.log('[v13.9] First hotspot full JSON:', JSON.stringify(allHotspots[0], null, 2));
            console.log('[v13.9] Total hotspots:', allHotspots.length);
            const h0 = allHotspots[0];
            console.log('[v13.9] Key fields:', {
                sst: h0.sst, do_surface: h0.do_surface, npp: h0.npp, depth_m: h0.depth_m,
                phi: h0.phi, z20_m: h0.z20_m, mld_m: h0.mld_m, eke: h0.eke,
                feeding_index: h0.feeding_index, greenfish_hsi: h0.greenfish_hsi,
                lunar_phase: h0.lunar_phase, hook_depth_advice: h0.hook_depth_advice,
                eddy_type_label: h0.eddy_type_label, precision_grade: h0.precision_grade,
                data_freshness: h0.data_freshness, shap_values: h0.shap_values,
                dvm_depth_m: h0.dvm_depth_m, distance_nm: h0.distance_nm,
            });
        }
        typhoonData = d.typhoons || [];
        // [v14.0] Fleet Operations
        window._optimizedRoute = d.optimized_route || {};
        window._currentShear = d.current_shear || {};
        ...
        displayHotspots.forEach(h => {
            h.score = Math.min(h.score || 0, 0.95);     // HSI max 95%
            h.npp = Math.min(h.npp || 0, 2500);          // NPP max 2500 mgC/m²/d
...
async function loadSea() {
    try {
        // [v13.2-R7] 8s fetch timeout
        const ctrl2 = new AbortController();
        const tid2 = setTimeout(() => ctrl2.abort(), 8000);
        const r = await fetch('/api/sea-conditions', { headers: authH(), signal: ctrl2.signal });
        clearTimeout(tid2);
        if (!r.ok) {
            // [v13.2-P2] Don't silently swallow errors
            if (r.status === 403) {
                setStatus('warn', 'API Key 已過期，請重新登入');
                console.error('loadSea: 403 Forbidden — API key expired');
            } else {
                setStatus('warn', `海況數據載入失敗 (${r.status})`);
                console.error(`loadSea: HTTP ${r.status}`);
            }
            return;
        }
        ...
        console.error('loadSea error:', e);
        setStatus('warn', '海況數據連線失敗');
    }
}
...
function renderMap() {
    ...
    spots.forEach((h, i) => {
        ...
        <div style="min-width:300px">
            ...
            <div style="font-size:11px;color:#7a8ca8;margin:6px 0">
                ...
                · <span style="color:${h.ml_type === '12-feature' ? '#f59e0b' : '#6b7280'};font-weight:600">${h.ml_type === '44-feature' ? '🤖 ML-44' : h.ml_type === '12-feature' ? '⚠️ ML-12 降級' : '📐 純物理'}</span>
            </div>
    ...
    // [v14.0] TSP Optimized Route — yellow dashed polyline
    if (window._optimizedRoute && window._optimizedRoute.geojson && typeof L !== 'undefined') {
        try {
            const coords = window._optimizedRoute.geojson.geometry.coordinates;
            if (coords && coords.length > 1) {
                const latLngs = coords.map(c => [c[1], c[0]]);
                L.polyline(latLngs, {
                    color: '#fbbf24', weight: 2.5, dashArray: '8, 6',
                    opacity: 0.85,
                }).addTo(hotspotLayer);
                // Day markers
                const route = window._optimizedRoute.route || [];
                route.forEach(stop => {
                    const dayIcon = L.divIcon({
                        html: `<div style="background:#fbbf24;color:#000;font-size:9px;font-weight:800;
                            width:20px;height:20px;border-radius:50%;display:flex;
                            align-items:center;justify-content:center;border:2px solid #fff;
                            box-shadow:0 1px 4px rgba(0,0,0,.5)">D${stop.day}</div>`,
                        iconSize: [20, 20], iconAnchor: [10, 10],
                    });
                    L.marker([stop.lat, stop.lon], { icon: dayIcon }).addTo(hotspotLayer);
                });
                console.log('[v14.0] TSP route rendered:', route.length, 'stops,',
                    window._optimizedRoute.total_distance_nm, 'nm');
            }
        } catch (e) { console.warn('TSP route render error:', e); }
    }
...
function renderCards() {
    ...
    // [v13.5] Precision grade badge helper
    const _pgBadge = (h) => {
        const g = h.precision_grade || 'C';
        const c = g === 'A' ? '#22c55e' : g === 'B' ? '#eab308' : '#6b7280';
        return `<span style="display:inline-block;padding:1px 5px;border-radius:3px;font-size:8px;font-weight:700;background:${c}22;color:${c};border:1px solid ${c}55;margin-left:6px">${g} ${h.precision_label || ''}</span>`;
    };
    const _freshBadge = (h) => {
        const df = h.data_freshness || {};
        const parts = [];
        if (df.sst_age_hours != null) parts.push(`SST ${df.sst_age_hours < 24 ? '<1天' : Math.round(df.sst_age_hours / 24) + '天前'}`);
        else parts.push(`SST: ${df.sst || '?'}`);
        if (df.chl_age_hours != null) parts.push(`Chl ${df.chl_age_hours < 24 ? '<1天' : Math.round(df.chl_age_hours / 24) + '天前'}`);
        return `<div style="font-size:8px;color:var(--txt3);margin-top:2px">📡 ${parts.join(' | ')}</div>`;
    };
    const _staleWarn = (h) => {
        if (!h.staleness_warning) return '';
        return `<div style="margin-top:4px;padding:4px 6px;background:rgba(234,179,8,.08);border-radius:4px;font-size:9px;color:#eab308;line-height:1.5">${h.staleness_warning}</div>`;
    };
    ...
    <div class="best-card" onclick="flyTo(${b.lat},${b.lon},0)">
        ...
        <div class="best-coord">
            <small>📍 精準座標 (建議航向) ${_pgBadge(b)}</small>
            ${(b.lat || 0).toFixed(3)}°N &nbsp; ${(b.lon || 0).toFixed(3)}°E
            ${_freshBadge(b)}
        </div>
        ${_staleWarn(b)}
        ...
        <div class="best-metrics">
            ...
            ${b.lunar_phase ? `<div class="bm"><div class="lab">${b.lunar_phase}</div><div class="val">${b.hook_depth_advice || ''}</div></div>` : ''}
            ${b.eddy_type_label ? `<div class="bm"><div class="lab">🌊 渦旋</div><div class="val">${b.eddy_type_label}</div></div>` : ''}
            ${b.shear_warning ? `<div class="bm"><div class="lab">${b.shear_risk_level === 2 ? '🔴' : '🟡'} 海流剪切</div><div class="val" style="color:${b.shear_risk_level === 2 ? 'var(--danger)' : 'var(--warn)'}"><b>${(b.current_shear_ms || 0).toFixed(2)} m/s</b></div></div>` : ''}
        </div>
    ...
    spots.slice(1).forEach((h, i) => {
        ...
        <div class="h-card ${tier}" onclick="flyTo(${h.lat},${h.lon},${idx})">
            <div class="hc-top">
                <div style="display:flex;align-items:center;gap:10px">
                    ...
                    <div>
                        ...
                        <div class="hc-coord">${(h.lat || 0).toFixed(3)}°N, ${(h.lon || 0).toFixed(3)}°E ${_pgBadge(h)}</div>
                    </div>
                </div>
            ...
            <div class="hc-grid">
                ...
                ${h.eddy_type_label ? `<span>🌊 ${h.eddy_type_label}</span>` : ''}
                ${h.lunar_phase ? `<span>${h.lunar_phase}</span>` : ''}
                ${h.shear_warning ? `<span style="color:${h.shear_risk_level === 2 ? 'var(--danger)' : 'var(--warn)'}">${h.shear_risk_level === 2 ? '🔴' : '🟡'} 剪切${(h.current_shear_ms || 0).toFixed(2)}</span>` : ''}
                <span style="color:${h.ml_type === '12-feature' ? 'var(--warn)' : 'var(--txt3)'};font-weight:${h.ml_type === '12-feature' ? '700' : '400'}">${h.ml_type === '44-feature' ? '🤖 ML-44' : h.ml_type === '12-feature' ? '⚠️ ML-12' : '📐 物理'}</span>
            </div>
            ${_freshBadge(h)}
            ${_staleWarn(h)}
...
function openShap(idx) {
    ...
    document.getElementById('shapBody').innerHTML = `
        ...
        <div style="margin:12px 0;padding:10px;border-radius:8px;background:rgba(0,212,255,.06);border:1px solid rgba(0,212,255,.15)">
            <div style="font-size:13px;font-weight:700;margin-bottom:8px">🧪 v15.3 生態鏈指標</div>
...
// ═══════════════════════════════════════════════════
//  [v13.5] Typhoon Visualization
// ═══════════════════════════════════════════════════
let lastTyphoonData = null;  // 保留最後一次颱風資料 (無颱風時灰化顯示)

async function loadTyphoons() {
    try {
        const ctrl = new AbortController();
        const tid = setTimeout(() => ctrl.abort(), 8000);
        const r = await fetch('/api/typhoon-status', { headers: authH(), signal: ctrl.signal });
        clearTimeout(tid);
        if (!r.ok) { console.warn('Typhoon API:', r.status); return; }
        const d = await r.json();
        if (d.active_typhoons > 0) {
            lastTyphoonData = d;
            renderTyphoonMap(d, false);
            updateTyphoonPanel(d, false);
        } else {
            renderTyphoonMap(lastTyphoonData, true);  // 灰化歷史路徑
            updateTyphoonPanel(null, true);
        }
    } catch (e) {
        console.warn('loadTyphoons error:', e);
        updateTyphoonPanel(null, true);
    }
}

function renderTyphoonMap(data, isHistory) {
    if (!map || !typhoonLayer) return;
    typhoonLayer.clearLayers();
    if (!data || !data.alerts) return;

    const opacity = isHistory ? 0.3 : 0.8;
    const prefix = isHistory ? '[歷史] ' : '';
    ...
    data.alerts.forEach(a => {
        ...
        if (typeof L !== 'undefined') {
            ...
            L.circle([a.lat, a.lon], {
                radius: 500000, color: '#eab308', fillColor: '#eab308',
                fillOpacity: 0.06 * (isHistory ? 0.3 : 1), weight: 1, opacity: opacity * 0.5,
                dashArray: '8,4',
            }).addTo(typhoonLayer);
            // 350km 橘色中度
            L.circle([a.lat, a.lon], {
                radius: 350000, color: '#f97316', fillColor: '#f97316',
                fillOpacity: 0.08 * (isHistory ? 0.3 : 1), weight: 1.5, opacity: opacity * 0.6,
                dashArray: '6,3',
            }).addTo(typhoonLayer);
            // 200km 紅色核心
            L.circle([a.lat, a.lon], {
                radius: 200000, color: '#ef4444', fillColor: '#ef4444',
                fillOpacity: 0.12 * (isHistory ? 0.3 : 1), weight: 2, opacity: opacity * 0.8,
            }).addTo(typhoonLayer);
            ...
            const icon = L.divIcon({
                ...
                html: `<div style="
                    ...
                    background:radial-gradient(circle, rgba(239,68,68,${isHistory ? 0.3 : 0.9}) 0%, rgba(239,68,68,0) 70%);
                    display:flex;align-items:center;justify-content:center;
                    font-size:20px;${isHistory ? '' : 'animation:spin 3s linear infinite;'}
                ">🌀</div>`,
                iconSize: [32, 32], iconAnchor: [16, 16],
            });
            L.marker([a.lat, a.lon], { icon })
                .bindPopup(`<div style="font-size:12px;line-height:1.6">
                    <b>${prefix}${a.name || '未命名'}</b> (${a.typhoon_id})<br>
                    風速: ${a.intensity_kt}kt | 等級: Cat-${cat}<br>
                    位置: ${a.lat.toFixed(1)}°N, ${a.lon.toFixed(1)}°E<br>
                    ${a.source ? '來源: ' + a.source : ''}
                </div>`)
                .addTo(typhoonLayer);

            // ── 預測路徑 (虛線) ──
            if (a.forecast_track && a.forecast_track.length > 1) {
                const coords = a.forecast_track.map(fp => [fp.lat, fp.lon]);
                L.polyline(coords, {
                    color: isHistory ? '#666' : '#ef4444',
                    weight: 2.5, dashArray: '10,6',
                    opacity: opacity,
                }).addTo(typhoonLayer);

                // 每 12h 預測點
                a.forecast_track.forEach(fp => {
                    if (fp.hour === 0) return;
                    const fpCat = fp.category || 0;
                    const fpColor = fpCat >= 3 ? '#ef4444' : fpCat >= 1 ? '#f97316' : '#eab308';
                    ...
                    const fpIcon = L.divIcon({
                        ...
                        html: `<div style="
                            ...
                            border:2px solid ${isHistory ? '#555' : fpColor};
                            background:${isHistory ? 'rgba(80,80,80,0.3)' : fpColor + '33'};
                            display:flex;align-items:center;justify-content:center;
                            font-size:7px;color:${isHistory ? '#888' : '#fff'};font-weight:700;
                        ">T${fp.hour}</div>`,
                        iconSize: [16, 16], iconAnchor: [8, 8],
                    });
                    L.marker([fp.lat, fp.lon], { icon: fpIcon })
                        .bindTooltip(`T+${fp.hour}h: ${fp.intensity_kt}kt (Cat-${fpCat})`, {
                    ...
                    if (!isHistory) {
                        L.circle([fp.lat, fp.lon], {
                            ...
                            fillOpacity: 0.03, weight: 1, opacity: 0.25,
                            dashArray: '4,8',
                        }).addTo(typhoonLayer);
                    }
                });
...
function updateTyphoonPanel(data, noTyphoon) {
    const body = document.getElementById('tyBody');
    if (!body) return;
    ...
    if (noTyphoon) {
        // 無颱風 — 顯示歷史 + 安全狀態
        let historyNote = '';
        if (lastTyphoonData && lastTyphoonData.alerts && lastTyphoonData.alerts.length > 0) {
            const last = lastTyphoonData.alerts[0];
            historyNote = `<div style="margin-top:8px;padding:8px;background:rgba(255,255,255,.03);border-radius:8px;font-size:10px;color:#556380;line-height:1.6">
                📋 上次追蹤: ${last.name || '未命名'} (${last.typhoon_id})<br>
                最後位置: ${last.lat.toFixed(1)}°N, ${last.lon.toFixed(1)}°E | ${last.intensity_kt}kt<br>
                <span style="color:#444">地圖上灰色路徑為歷史軌跡</span>
            </div>`;
        }
        body.innerHTML = `
            <div style="color:var(--ok);text-align:center;padding:16px;font-size:13px">
                ✅ 目前無颱風警報<br>
                <span style="font-size:10px;color:var(--txt3)">所有推薦海域安全無虞</span>
            </div>
            ${historyNote}
        `;
        return;
    ...
    if (!data || !data.alerts || data.alerts.length === 0) {
        body.innerHTML = '<div style="color:var(--txt3);text-align:center;padding:20px">✅ 目前無颱風警報</div>';
        return;
    }

    body.innerHTML = data.alerts.map(a => {
        const cat = a.category || 0;
        const catColor = cat >= 3 ? '#ef4444' : cat >= 1 ? '#f97316' : '#eab308';
        const catLabel = cat >= 4 ? '超強颱風' : cat >= 3 ? '強烈颱風' : cat >= 1 ? '中度颱風' : '輕度颱風';

        // 白話說明
        let warning = '';
        if (a.forecast_track && a.forecast_track.length > 1) {
            const fp48 = a.forecast_track.find(fp => fp.hour === 48) || a.forecast_track[a.forecast_track.length - 1];
            const fp72 = a.forecast_track[a.forecast_track.length - 1];
            const areaName = a.lat > 25 ? '東海' : a.lat > 20 ? '台灣東部外海' : a.lat > 15 ? '巴士海峽' : '菲律賓海';
            const futureArea = fp48.lat > 25 ? '東海/日本南方' : fp48.lat > 20 ? '台灣附近海域' : '菲律賓東方海域';

            warning = `<div style="margin-top:8px;padding:8px;background:rgba(239,68,68,.08);border-radius:8px;font-size:11px;color:#fca5a5;line-height:1.8">
                ⚠️ <b>${a.name}</b> 目前位於<b>${areaName}</b>，未來 48 小時將移向<b>${futureArea}</b>。<br>
                🚢 建議迴避 ${a.name} 中心 350km 以內海域，預計路徑沿線熱點已自動降級或剔除。<br>
                📉 72h 後預估強度: ${fp72.intensity_kt}kt (${fp72.category >= 1 ? 'Cat-' + fp72.category : '熱帶性低氣壓'})
            </div>`;
        }

        return `<div style="padding:10px;border-bottom:1px solid rgba(255,255,255,.06)">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <span style="font-size:18px;animation:spin 3s linear infinite">🌀</span>
                <div>
                    <div style="font-size:13px;font-weight:700;color:${catColor}">${a.name || '未命名'}</div>
                    <div style="font-size:10px;color:var(--txt3)">${a.typhoon_id} | ${catLabel} | ${a.source}</div>
                </div>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;font-size:10px;color:var(--txt2)">
                <div>📍 ${a.lat.toFixed(1)}°N, ${a.lon.toFixed(1)}°E</div>
                <div>💨 ${a.intensity_kt}kt (Cat-${cat})</div>
            </div>
            ${warning}
        </div>`;
    }).join('');
...
setInterval(loadTyphoons, 600000);  // 每 10 分鐘更新颱風
