...
try:
    ...
    log.debug(f"[降級] ml_system/integrated_predictor.py: {e}")
...
class IntegratedPredictor:
    ...
    def load_ml_model(self, model_dir: str = "models"):
        """Load trained ML model and feature pipeline. [v13.2-P0] With HMAC verification."""
        path = Path(model_dir)
        sp = self.species

        # [v13.2-P0] HMAC verification
        try:
        ...
        try:
            ...
            pipe_path = path / f"feature_pipeline_{sp}.pkl"
            ...
            log.error(f"[P0] ML model signature verification failed: {e}")
            self.ml_available = False
