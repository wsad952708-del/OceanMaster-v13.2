...
class FeatureEngineeringPipeline:
    ...
    BASE_FEATURES = [
        ...
        # [海鷹/蒼鷺] 深層特徵
        "t100", "gradient_strength", "delta_t_surface_100", "chl_lag15d",
    ]
    ...
    DERIVED_FEATURES = [
        ...
        # [海鷹] 深層衍生
        "thermal_gradient_index",  # t100 * gradient_strength
        "thermocline_quality",     # delta_t / (gradient_strength + 0.01)
    ]
    ...
    def transform(
        ...
        # 4b. [海鷹] 深層衍生特徵
        features["thermal_gradient_index"] = (
            features["t100"] * features["gradient_strength"]
        )
        features["thermocline_quality"] = (
            features["delta_t_surface_100"] / (features["gradient_strength"] + 0.01)
        )
    ...
    def save(self, path: str = "models/feature_pipeline.pkl"):
        """Save fitted pipeline to disk. [v13.2-P0] With HMAC signing."""
        ...
        try:
            ...
            _sign_model_file(Path(path))
        except ImportError as e:
            log.debug(f"[降級] ml_system/feature_engineering_pipeline.py: {e}")
        log.info(f"Pipeline saved: {path}")

    def load(self, path: str = "models/feature_pipeline.pkl"):
        """Load fitted pipeline from disk. [v13.2-P0] With HMAC verification."""
        # [v13.2-P0] Verify before loading
        try:
            from engine.ml.stacking_ensemble import _verify_model_file
            _verify_model_file(Path(path))
        except ImportError:
