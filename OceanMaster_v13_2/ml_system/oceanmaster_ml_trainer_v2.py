...
class OceanMasterMLTrainer:
    ...
    def train(
        ...
        Args:
            ...
            lats: Sample latitudes (for SpatialBlockCV)
            lons: Sample longitudes (for SpatialBlockCV)
        ...
        # Cross-validation: SpatialBlockCV if coords available, else KFold
        ...
        if lats is not None and lons is not None:
            ...
            sp_result = spatial_cv.cross_val_score(self.model, X, y, lats, lons)
            cv_r2 = sp_result["r2_mean"]
            cv_mae = sp_result.get("rmse_mean", 0)  # approximate
            cv_mae_std = sp_result.get("r2_std", 0)
            log.info(f"  SpatialBlockCV R2={cv_r2:.4f}")
        else:
            kf = KFold(
    ...
    def save(self, model_dir: str = "models"):
        """Save trained model and all artifacts. [v13.2-P0] With HMAC signing."""
        path = Path(model_dir)
        path.mkdir(parents=True, exist_ok=True)

        sp = self.config.species

        # [v13.2-P0] Import signing
        try:
            from engine.ml.stacking_ensemble import _sign_model_file
        except ImportError:
            _sign_model_file = lambda p: None

        # Model
        model_path = path / f"stacking_{sp}.pkl"
        joblib.dump(self.model, model_path)
        _sign_model_file(model_path)

        # Scaler (if separate from pipeline)
        scaler_path = path / f"scaler_{sp}.pkl"
    ...
    def load(self, model_dir: str = "models"):
        """Load trained model from disk. [v13.2-P0] With HMAC verification."""
        path = Path(model_dir)
        sp = self.config.species

        # [v13.2-P0] Verify before loading
        try:
            from engine.ml.stacking_ensemble import _verify_model_file
        except ImportError:
            log.warning("[P1] stacking_ensemble not available — model signature verification DISABLED")
            _verify_model_file = lambda p: True
