"""
OceanMaster v12 — Phase 9: End-to-End Training & Validation  # [v12-phase9]
=============================================================================
...
ALL_SPECIES = ["yellowfin", "bigeye", "albacore", "skipjack"]
...
def train_species(
    ...
    fao_path = Path(f"data/fao_cpue_{species}.csv")
    fao_generic = Path("data/fao_cpue.csv")

    if fao_path.exists():
        log.info(f"Using real FAO data: {fao_path}")
        from engine.ml.fao_data_loader import FAODataLoader
        loader = FAODataLoader()
        X, y, feature_names = loader.get_training_ready(str(fao_path), species)
    elif fao_generic.exists():
        log.info(f"Using real FAO data: {fao_generic}")
        from engine.ml.fao_data_loader import FAODataLoader
        loader = FAODataLoader()
        X, y, feature_names = loader.get_training_ready(str(fao_generic), species)
    else:
        log.info(f"Using synthetic data (swap for real FAO data later)")
        seed_val = 42 + sum(ord(c) for c in species)
        gen = SyntheticCPUEGenerator(seed=seed_val)
        X, y, feature_names = gen.generate_full_dataset(n_samples, species)

    # ── Step 2: Validate feature alignment ──
    expected = list(FeatureEngineer.FEATURE_NAMES)
    assert len(feature_names) == len(expected), (
        f"Feature count mismatch: got {len(feature_names)}, expected {len(expected)}"
    )
    assert feature_names == expected, (
        f"Feature names mismatch:\n"
        f"  Got:      {feature_names[:5]}...\n"
...
def main():
    parser = argparse.ArgumentParser(description="OceanMaster v12 ML Training")
    ...
    log.info("╔══════════════════════════════════════════════════════╗")
    log.info("║  OceanMaster v12 — ML Training Pipeline (Phase 9)  ║")
    log.info("╚══════════════════════════════════════════════════════╝")
