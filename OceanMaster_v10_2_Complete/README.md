# 🌊 OceanMaster v12 — AI-Powered Tuna Fishing Ground Prediction System
...
The system covers **4 tuna species** — yellowfin, bigeye, albacore, and skipjack — with species-specific habitat models grounded in peer-reviewed fisheries oceanography. A **stacking ensemble** (Random Forest + XGBoost + LightGBM + Ridge meta-learner) provides ML-based CPUE predictions, fused at a 40/60 ratio with science-based Habitat Suitability Index (HSI) scores.
...
- **44-feature model**: Incorporates SST, Chl-a fractions, DVM, lunar phase, OMZ, eddies, fronts, ENSO, and more
...
- **Stacking Ensemble**: Random Forest + XGBoost + LightGBM + Ridge meta-learner
- **Optional CatBoost** 5th base learner
- **44-feature model** with automatic 12-feature legacy fallback
- **Hybrid prediction**: 40% science-based HSI + 60% ML prediction (masked by Φ metabolic index)
- **SHAP explainability** for every prediction
- **Safe model loading** with 5-point validation (pickle integrity, species match, scaler, feature count, meta)
...
```bash
git clone https://github.com/your-org/oceanmaster.git
cd oceanmaster
cp .env.example .env
...
1. **Synthetic training data**: ML models are trained on scientifically realistic generated data, not real catch records. Production deployment should retrain on FAO/RFMO CPUE data.
2. **No field validation**: Predictions have not been validated against actual fishing vessel catches in real-world conditions.
3. **Internet dependency**: All satellite data requires active internet connection. Offline operation uses cached data (configurable TTL).
4. **Prediction accuracy**: R² scores (0.88–0.91) reflect synthetic data performance. Real-world accuracy will be lower and has not been independently verified.
5. **Regional focus**: Default configuration targets Western & Central Pacific (120°E–180°, 10°S–40°N). Other regions require parameter adjustment.
