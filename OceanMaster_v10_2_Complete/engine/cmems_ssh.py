"""
OceanMaster v9.0 — CMEMS SSH 數據擷取 + 渦旋偵測
===================================================
...
class CMEMSFetcher:
    ...
    def __init__(
        ...
        # 嘗試導入 copernicusmarine
        try:
            import copernicusmarine
            self._has_cmems_api = True
        except ImportError:
            self._has_cmems_api = False
            log.info("copernicusmarine 未安裝，使用 AVISO+ HTTP 備用方式")
    ...
    async def _fetch_via_cmems_api(self) -> Optional[Dict[str, np.ndarray]]:
        """透過 copernicusmarine API 取得 SSH"""
        try:
            import copernicusmarine
            import xarray as xr
            ...
            log.info("透過 CMEMS API 取得 SSH 數據...")

            # open_dataset 是 lazy loading，不會下載全部數據
    ...
    async def _fetch_via_aviso_erddap(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            ...
            # AVISO gridded SLA (0.25°, 日) — 免費公開
            url = (
                f"https://coastwatch.pfeg.noaa.gov/erddap/griddap/"
                f"erdTAgeo1day_LonPM180.json"
                f"?sla[({t_start}):1:({t_end})]"
                f"[({self.lat_min}):1:({self.lat_max})]"
                f"[({self.lon_min}):1:({self.lon_max})]"
            )

            log.info("透過 AVISO ERDDAP 取得 SLA 數據...")

            from engine.data_fetcher_v2 import safe_fetch
            async with httpx.AsyncClient() as client:
                resp = await safe_fetch(client, url, "AVISO-SLA", timeout=120)
            ...
            sla_idx = None
            for candidate in ["sla", "sea_level_anomaly", "adt"]:
            ...
            sla_grid = sla_latest.reshape(len(lats), len(lons))
    ...
    def _generate_demo_ssh(self) -> Dict[str, np.ndarray]:
        ...
        np.random.seed(42)
        for _ in range(8):
            cy = np.random.uniform(self.lat_min + 5, self.lat_max - 5)
            cx = np.random.uniform(self.lon_min + 5, self.lon_max - 5)
            r = np.random.uniform(1, 3)
            amp = np.random.uniform(-0.15, 0.15)
            ssh_base += amp * np.exp(-((lat_g - cy)**2 + (lon_g - cx)**2) / (2 * r**2))

        ssh_base += np.random.normal(0, 0.02, ssh_base.shape)
