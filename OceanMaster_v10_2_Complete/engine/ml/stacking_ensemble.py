"""
OceanMaster v12 — ML Stacking Ensemble 融合引擎  # [v12-enhance]
=================================================
...
class FeatureEngineer:
    ...
    def extract_features(
        ...
        features["sst_local_std"] = ndimage.generic_filter(
            sst_filled, np.std, size=5
        ).ravel()
    ...
    def _compute_distance_field(
        ...
        用於生成 dist_to_front, dist_to_eddy 等特徵。
        """
        ny, nx = mask.shape
        ...
        for j in range(ny):
            ...
            for i in range(nx):
                ...
                min_d = 500.0
                for fj, fi in feature_pts:
                    if fj < len(lats) and fi < len(lons):
                        # 快速近似距離 (km)
                        dlat = (lats[fj] - lat1) * 111.0
                        dlon = (lons[fi] - lon1) * 111.0 * np.cos(np.radians(lat1))
    ...
    def safe_model_load(self) -> bool:
        ...
        if scaler is not None:
            ...
            if n_features_saved != n_features_current:
                log.warning(
                    f"[v12-fix] 特徵數不匹配: 模型={n_features_saved}, "
                    f"當前={n_features_current}。模型可能需要重新訓練。"
                )
                return False
